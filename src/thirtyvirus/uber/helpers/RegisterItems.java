...
public class RegisterItems {
    ...
    public static void registerUberItems() {
        ...
        UberItems.putItem("builders_wand", new builders_wand(1, UberRarity.LEGENDARY, "Builder's Wand", XMaterial.STICK.parseMaterial(),  false, false, true, Arrays.asList(new UberAbility("Contruction!", AbilityType.RIGHT_CLICK, "Right click the face of any block to\nextend all connected block faces.\n" + ChatColor.DARK_GRAY + "(consumes blocks from your inventory)")), null));
        ...
        UberItems.putItem("escape_rope", new escape_rope(5, UberRarity.UNCOMMON,  "Escape Rope", XMaterial.LEAD.parseMaterial(),true, true, true, Arrays.asList(new UberAbility("Round Trip!", AbilityType.RIGHT_CLICK, "Instantly teleport back to the last\nlocation with the sky visible")), null));
        UberItems.putItem("fireball", new fireball(6, UberRarity.UNCOMMON,  "FireBall", XMaterial.FIRE_CHARGE.parseMaterial(),  true, true, false, Arrays.asList(new UberAbility("Throw em!", AbilityType.RIGHT_CLICK, "Throw a fireball which\nexplodes on impact")), null));
        ...
        UberItems.items.get("shooty_box").setCraftingRecipe(new UberCraftingRecipe(Arrays.asList(
