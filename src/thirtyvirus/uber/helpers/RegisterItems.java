...
public class RegisterItems {
    ...
    public static void registerUberItems() {
        ...
        UberItems.putItem("builders_wand", new builders_wand(1, UberRarity.LEGENDARY, "Builder's Wand", Material.STICK,  false, false, true, Arrays.asList(new UberAbility("Contruction!", AbilityType.RIGHT_CLICK, "Right click the face of any block to extend all connected block faces.\n " + ChatColor.DARK_GRAY + "(consumes blocks from your inventory)")), null));
        UberItems.putItem("lunch_box", new lunch_box(2, UberRarity.RARE, "Lunch Box", Material.CONDUIT,  false, false, false, Arrays.asList(new UberAbility("Gluttony", AbilityType.NONE, "Automatically feeds you when hungry\n " + ChatColor.DARK_GRAY + "(drag & click food onto box to fill)")), null));
        UberItems.putItem("document_of_order", new document_of_order(3, UberRarity.EPIC, "Document of Order", Material.PAPER, false, false, false, Arrays.asList(new UberAbility("Bureaucracy", AbilityType.RIGHT_CLICK, "Use on a container block while crouched to sort that container's contents. Crouch use on air to sort own inventory."), new UberAbility("Multisort", AbilityType.LEFT_CLICK, "Select many containers at once, then left click any non-container block to confirm, cr crouch left click any block to cancel. "+ ChatColor.DARK_GRAY + "(sorts everything as if one large inventory)")), null));
